...
import { InvoiceDetails } from "@/app/invoices/_components/details";
...
jest.mock("@/app/invoices/_components/classification-feedback", () => ({
  ClassificationFeedback: () => <div data-testid="classification-feedback">Feedback</div>,
}));

jest.mock("@/app/invoices/_components/classification-assigned", () => ({
  __esModule: true,
  default: () => <div data-testid="classification-assigned">Assigned</div>,
}));

jest.mock("@/app/invoices/_components/edit-payment-dialog", () => ({
...
jest.mock("lucide-react", () => ({
  ...
  Link2: () => <div data-testid="link2">Link2</div>,
}));
...
describe("InvoiceDetails Component Integration", () => {
  ...
  it("should render 'Vincular Pago' button and open dialog when clicked", async () => {
    render(<InvoiceDetails data={mockInvoice} relatedPayments={[]} />);

    const linkBtn = screen.getByText("Vincular Pago");
    expect(linkBtn).toBeInTheDocument();

    // Click button to open link payment dialog
    fireEvent.click(linkBtn);

    expect(screen.getByTestId("link-payment-complement-dialog")).toBeInTheDocument();
  });
});
